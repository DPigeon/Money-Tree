...
describe('StoreFacadeService', () => {
  ...
  it('should dispatach create new user', () => {
    ...
    expect(spy).toHaveBeenCalledWith(appActions.createNewUser({ user: userInfo }));
  })
  ...
  it('should dispatach update user', () => {
    ...
    expect(spy).toHaveBeenCalledWith(appActions.upadateUser({ user: userInfo }));
  })

  it('should dispatach get current user', () => {
    const spy = jest.spyOn(store, 'dispatch');
    service.getCurrentUser(userInfo.id)
    expect(spy).toHaveBeenCalledWith(appActions.getCurrentUser({ id: userInfo.id }));
  })
